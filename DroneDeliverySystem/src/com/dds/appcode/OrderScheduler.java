...
public class OrderScheduler {
	...
	public void orderSelection(ArrayList<OrderDetails> sortedTripOrderDetails,
			ArrayList<OrderDetails> custOrderDetails) {
		...
		while (order != null) {
			...
			// the customer
			deliveryTime = getDeliveryTime(order, this.currDispatchTime);

			// when the drone comes back to the base;
			returnTime = getDeliveryTime(order, this.currDispatchTime);
			...
			order = selectNextOrder(sortedTripOrderDetails, custOrderDetails);
